...
class FirestoreTester {
  ...
  Future<void> testDatabaseSetup() async {
    ...
    try {
      ...
      print('\n❌ Test failed: $e');
      print('💡 Make sure Firebase is properly initialized and you have the correct permissions.');
    }
  }
  ...
  Future<void> _testAdminsCollection() async {
    try {
      ...
      for (var doc in admins.docs) {
        ...
        print('   - ${data['name']} (${data['email']}) - Active: ${data['isActive']}');
      }
      print('');
    } catch (e) {
  ...
  Future<void> _testInstructorsCollection() async {
    try {
      QuerySnapshot instructors = await _firestore.collection('instructors').get();
      print('👨‍🏫 Instructors Collection:');
      print('   - Total instructors: ${instructors.docs.length}');
      
      for (var doc in instructors.docs) {
        Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
        print('   - ${data['name']} (${data['email']}) - Department: ${data['department']}');
  ...
  Future<void> _testUsersCollection() async {
    try {
      ...
      for (var doc in users.docs) {
        Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
        print('   - ${data['fullName']} (${data['email']}) - Role: ${data['role']} - Points: ${data['points'] ?? 0}');
      }
      print('');
    } catch (e) {
  ...
  Future<void> _testClassesCollection() async {
    try {
      ...
      for (var doc in classes.docs) {
        Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
        print('   - ${data['course']} - ${data['section']} - ${data['day']} ${data['startTime']}-${data['endTime']}');
  ...
  Future<void> _testActivitiesCollection() async {
    try {
      QuerySnapshot activities = await _firestore.collection('activities').get();
      print('🌱 Activities Collection:');
      print('   - Total activities: ${activities.docs.length}');
      
      for (var doc in activities.docs) {
        Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
        print('   - ${data['title']} - Status: ${data['status']} - Points: ${data['pointsReward']}');
        if (data['registeredParticipants'] != null) {
          print('     Participants: ${(data['registeredParticipants'] as List).length}');
        }
      }
      print('');
    } catch (e) {
  ...
  Future<void> _testAnnouncementsCollection() async {
    try {
      QuerySnapshot announcements = await _firestore.collection('announcements').get();
      print('📢 Announcements Collection:');
      print('   - Total announcements: ${announcements.docs.length}');
      
      for (var doc in announcements.docs) {
        Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
        print('   - ${data['title']} - Priority: ${data['priority']} - Active: ${data['isActive']}');
      }
      print('');
    } catch (e) {
  ...
  Future<void> _testLeaderboardCollection() async {
    try {
      QuerySnapshot leaderboard = await _firestore.collection('leaderboard').get();
      ...
      sortedDocs.sort((a, b) => (a.data() as Map<String, dynamic>)['rank'].compareTo((b.data() as Map<String, dynamic>)['rank']));
      
      for (var doc in sortedDocs) {
        Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
        print('   - Rank ${data['rank']}: ${data['name']} - ${data['points']} points - Level ${data['level']}');
      }
      print('');
    } catch (e) {
  ...
  Future<void> generateTestReport() async {
    ...
    try {
      ...
      List<String> collections = ['users', 'admins', 'instructors', 'classes', 'activities', 'announcements', 'leaderboard'];
