...
class WebActivityDetailScreen extends StatefulWidget {
  ...
  const WebActivityDetailScreen({super.key, required this.activity});
...
class _WebActivityDetailScreenState extends State<WebActivityDetailScreen> {
  ...
  void initState() {
    ...
    try {
      ...
      activityController = Get.put(ActivityController(), permanent: true);
    }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // Back button
              TextButton.icon(
                onPressed: () => Get.toNamed(WebRoutes.activities),
        ...
        Row(
          children: [
            _buildInfoChip(
              ...
              widget.activity['instructorName'] ?? 'Instructor',
            ),
            const SizedBox(width: 24),
            _buildInfoChip(
              Icons.calendar_today_outlined,
              'Due: ${activityController.getDueDateFormatted(widget.activity['dueDate'])}',
            ),
          ],
        ),
      ],
  ...
  Widget _buildPointsBadge() {
    return Row(
      children: [
        ...
        const SizedBox(width: 8),
        Text(
          '${widget.activity['points'] ?? 0} Points',
  ...
  Widget _buildSubmissionAction() {
    final isPastDue = DueDateUtils.isPastDue(widget.activity['dueDate']);
    return Obx(() {
      final isSubmitted =
